...
import { MapPin, Phone, Mail, Clock, Instagram, Facebook, Twitter, Menu as MenuIcon, X, ChevronRight, Calendar, Users, Sparkles, ChevronLeft, ArrowRight, Settings, Plus, Trash2, Save, Image as ImageIcon, Home as HomeIcon } from 'lucide-react';
...
const INITIAL_DATA = {
  assets: {
    logo: '302420978_466282258849212_2212484648470563221_n.jpg',
    coverPhotos: [
      '476892662_1153875933423171_7275817819802918023_n.jpg',
      'https://images.unsplash.com/photo-1514933651103-005eec06c04b?auto=format&fit=crop&q=80&w=2000' // Added second image for slideshow preview
    ],
  ...
  menu: [
    ...
    { id: 3, category: 'Starters', name: 'Wild Mushroom Tart', desc: 'Foraged mushrooms, thyme-infused cream, flaky butter pastry.', price: '₹520', img: 'https://images.unsplash.com/photo-1604543519952-16dc448d3db6?auto=format&fit=crop&q=80&w=300', isSpecial: false, showOnHome: false },
    ...
    { id: 5, category: 'Mains', name: 'Earth & Fire Grill', desc: 'Wood-fired cottage cheese or premium cut, char-grilled asparagus, smoked garlic mash.', price: '₹890', img: 'https://images.unsplash.com/photo-1544025162-811114a8f94c?auto=format&fit=crop&q=80&w=300', isSpecial: false, showOnHome: false },
    { id: 6, category: 'Mains', name: 'Lotus Stem & Water Chestnut', desc: 'Crispy lotus stem, wok-tossed water chestnuts, sweet chili soy glaze, steamed jasmine rice.', price: '₹680', img: 'https://images.unsplash.com/photo-1512058564366-18510be2db19?auto=format&fit=crop&q=80&w=300', isSpecial: false, showOnHome: false },
  ...
  gallery: [
    ...
    { id: 2, src: "https://images.unsplash.com/photo-1544148103-0773bf10d330?auto=format&fit=crop&q=80&w=1000", category: 'Food', showOnHome: false },
    { id: 3, src: "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&q=80&w=1000", category: 'Ambiance', showOnHome: true },
    { id: 4, src: "https://images.unsplash.com/photo-1559339352-11d035aa65de?auto=format&fit=crop&q=80&w=1000", category: 'Food', showOnHome: true },
    { id: 5, src: "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&q=80&w=1000", category: 'Ambiance', showOnHome: true },
    { id: 6, src: "https://images.unsplash.com/photo-1551218808-94e220e084d2?auto=format&fit=crop&q=80&w=1000", category: 'Food', showOnHome: false },
    { id: 7, src: "https://images.unsplash.com/photo-1600891964092-4316c288032e?auto=format&fit=crop&q=80&w=1000", category: 'Food', showOnHome: false },
    { id: 8, src: "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&q=80&w=1000", category: 'Ambiance', showOnHome: false },
  ]
};
  ...
  return (
    <div className="min-h-screen font-sans" style={{ backgroundColor: THEME.background, color: THEME.text }}>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          >
            <img 
              ...
              className={`transition-all duration-500 rounded-full object-cover ${isScrolled || currentPage !== 'home' ? 'h-10 w-10' : 'h-14 w-14 shadow-lg'}`}
            />
            <div 
      ...
      {isReservationOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl relative">
            <button onClick={() => setIsReservationOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-800 transition-colors z-10"><X size={24} /></button>
            <div className="bg-gray-50 p-8 text-center border-b border-gray-100">
              <h2 className="text-3xl font-serif mb-2" style={{ color: THEME.primary }}>Reserve Your Table</h2>
              <p className="text-sm opacity-70">Join us for an unforgettable dining experience.</p>
            </div>
            <form className="p-8 space-y-6" onSubmit={(e) => { e.preventDefault(); alert("Table Reserved! We look forward to hosting you at Nalla Bhoomi."); setIsReservationOpen(false); }}>
              <div className="grid grid-cols-2 gap-4">
                <div className="relative"><Calendar className="absolute left-3 top-3 opacity-40" size={20} /><input type="date" className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#5a992a] transition-colors" required /></div>
                <div className="relative"><Clock className="absolute left-3 top-3 opacity-40" size={20} /><input type="time" className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#5a992a] transition-colors" required /></div>
              </div>
              <div className="relative">
                <Users className="absolute left-3 top-3 opacity-40" size={20} />
                <select className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#5a992a] transition-colors" required>
                  <option value="">Number of Guests</option>
                  {[1,2,3,4,5,6,7,8].map(num => <option key={num} value={num}>{num} {num === 1 ? 'Guest' : 'Guests'}</option>)}
                  <option value="9+">9+ Guests</option>
                </select>
              </div>
              <input type="text" placeholder="Your Name" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#5a992a] transition-colors" required />
              <input type="tel" placeholder="Phone Number" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#5a992a] transition-colors" required />
              <button type="submit" className="w-full py-4 text-white uppercase tracking-widest text-sm font-semibold rounded-lg hover:opacity-90 transition-opacity mt-4 shadow-lg shadow-[#5a992a]/30" style={{ backgroundColor: THEME.accent }}>Confirm Reservation</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl relative p-8">
            ...
            <div className="flex justify-center mb-6">
              <img src={siteData.assets.logo} alt="Logo" className="w-16 h-16 rounded-full" />
            </div>
      ...
      <main className="min-h-screen">
        ...
        {currentPage === 'contact' && <Contact data={siteData} />}
        {currentPage === 'admin' && isAdmin && <AdminDashboard data={siteData} updateData={setSiteData} logout={() => { setIsAdmin(false); navigateTo('home'); }} />}
      </main>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 text-center md:text-left items-center md:items-start">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-center md:justify-start space-x-6 mt-6">
                <a href="#" className="hover:text-[#5a992a] transition-colors"><Instagram /></a>
                <a href="#" className="hover:text-[#5a992a] transition-colors"><Facebook /></a>
                <a href="#" className="hover:text-[#5a992a] transition-colors"><Twitter /></a>
              </div>
            </div>
          </div>
...
function Home({ navigateTo, openReservation, data }) {
  // Pull featured items and gallery strictly from global state
  ...
  // Slideshow State for Cover Photos
    ...
    <div className="animate-fadeIn">
      ...
      <div className="relative h-screen flex items-center justify-center">
        <div className="absolute inset-0">
          <img src={data.assets.heroBg} alt="Restaurant Interior" className="w-full h-full object-cover" />
      ...
      <div className="w-full bg-[#f4f0eb] py-16 border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-6 text-center">
           ...
           <p className="mt-4 text-xl font-serif italic text-gray-600 max-w-2xl mx-auto">
      ...
      <div className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative">
            <img src={data.assets.philosophyImg} alt="Signature Dish" className="w-full h-auto object-cover rounded-tl-[100px] rounded-br-[100px] shadow-2xl" />
      ...
      <div className="py-24 px-6 md:px-12 bg-[#fcfaf7] border-t border-gray-100">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            ...
            {featuredItems.length === 0 && <p className="col-span-3 text-center opacity-50 italic">No featured items selected. Go to Admin to select them.</p>}
          </div>
        </div>
      </div>
      ...
      <div className="py-24 px-6 md:px-12 max-w-7xl mx-auto border-t border-gray-100">
         <div className="flex flex-col md:flex-row justify-between items-end mb-12 space-y-4 md:space-y-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {homeGalleryImages.length === 0 && <p className="col-span-4 text-center opacity-50 italic">No featured gallery images selected. Go to Admin to select them.</p>}
          </div>
      </div>
    </div>
  );
...
function Menu({ data }) {
  ...
  // Dynamically extract categories from menu state
  const tabs = ['All', ...Array.from(new Set(data.menu.map(item => item.category)))];
  
  const displayedItems = activeTab === 'All' 
    ? data.menu
    : data.menu.filter(c => c.category === activeTab);

  return (
    <div className="pt-32 pb-24 px-6 md:px-12 max-w-5xl mx-auto animate-fadeIn">
      ...
      <div className="max-w-4xl mx-auto mb-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-50/80 p-2 rounded-2xl border border-gray-200 shadow-inner">
          {tabs.map((tab, index) => (
            <button
              key={index}
              onClick={() => setActiveTab(tab)}
              className={`w-full py-4 rounded-xl text-sm md:text-base font-semibold tracking-widest uppercase transition-all duration-300 ${
...
function Gallery({ data }) {
  ...
  const showNext = (e) => {
    e.stopPropagation();
    setSelectedImgIndex((prev) => (prev + 1) % filteredImages.length);
  };
  
  const showPrev = (e) => {
    e.stopPropagation();
    setSelectedImgIndex((prev) => (prev - 1 + filteredImages.length) % filteredImages.length);
  };
...
function Contact({ data }) {
  return (
    <div className="pt-32 pb-0 animate-fadeIn bg-white">
      <div className="max-w-7xl mx-auto px-6 md:px-12 mb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div className="space-y-12 bg-[#fcfaf7] p-8 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <form className="space-y-6 pt-8 border-t border-gray-200" onSubmit={(e) => e.preventDefault()}>
              <h3 className="text-2xl font-serif mb-6" style={{ color: THEME.primary }}>Send a Message</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input type="text" placeholder="Your Name" className="w-full pb-3 border-b border-gray-300 bg-transparent focus:outline-none focus:border-[#731c55] transition-colors" />
                <input type="email" placeholder="Email Address" className="w-full pb-3 border-b border-gray-300 bg-transparent focus:outline-none focus:border-[#731c55] transition-colors" />
              </div>
              <textarea placeholder="Your Message" rows="4" className="w-full pb-3 border-b border-gray-300 bg-transparent focus:outline-none focus:border-[#731c55] transition-colors resize-none"></textarea>
              <button className="px-10 py-4 text-white uppercase tracking-widest text-sm rounded-lg hover:opacity-90 transition-opacity w-full sm:w-auto shadow-md" style={{ backgroundColor: THEME.primary }}>Send Message</button>
            </form>
          </div>

          <div className="h-[700px] w-full bg-gray-100 rounded-3xl overflow-hidden shadow-xl relative border-4 border-white">
...
function AdminDashboard({ data, updateData, logout }) {
  ...
  const handleImageUpload = (e, callback) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        callback(reader.result); // Converts local file to Base64 string
      };
      reader.readAsDataURL(file);
    }
  };

  // Input handlers for General/Contact
  const handleAssetChange = (e) => updateData({ ...data, assets: { ...data.assets, [e.target.name]: e.target.value } });
  
  // Handler for direct file uploads in General Settings
  const handleAssetFileChange = (key) => (e) => {
    handleImageUpload(e, (base64) => {
      updateData({ ...data, assets: { ...data.assets, [key]: base64 } });
    });
  };
  ...
  const handleAddMenu = (e) => {
    e.preventDefault();
    updateData({ ...data, menu: [{ ...newMenu, id: Date.now(), showOnHome: false }, ...data.menu] });
    setNewMenu({ name: '', desc: '', price: '', img: '', category: 'Starters', isSpecial: false });
  };
  const toggleMenuHome = (id) => {
    updateData({ ...data, menu: data.menu.map(m => m.id === id ? { ...m, showOnHome: !m.showOnHome } : m) });
  };
  ...
  const toggleGalleryHome = (id) => {
    updateData({ ...data, gallery: data.gallery.map(g => g.id === id ? { ...g, showOnHome: !g.showOnHome } : g) });
  };
  const deleteGallery = (id) => updateData({ ...data, gallery: data.gallery.filter(g => g.id !== id) });

  return (
    <div className="pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto animate-fadeIn min-h-screen">
      <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-xl mb-8 flex items-start gap-3">
        <Settings className="flex-shrink-0 mt-1" />
        <div className="flex-1">
          <h3 className="font-bold">Admin Dashboard Mode</h3>
          <p className="text-sm opacity-90">Changes made here update the live website immediately. Since this is an in-memory preview, changes will reset if you refresh the page.</p>
        </div>
                    ...
                    return (
                      <div key={key} className="space-y-2">
                        ...
                        <div className="flex flex-col xl:flex-row gap-2">
                          <input type="text" name={key} value={value.startsWith('data:') ? 'Local File Uploaded' : value} onChange={handleAssetChange} className="flex-1 p-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#731c55] text-sm" placeholder="Image URL" readOnly={value.startsWith('data:')} />
            ...
            <div className="space-y-10 animate-fadeIn">
               <section className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                 ...
                 <form onSubmit={handleAddMenu} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input type="text" placeholder="Dish Name" required value={newMenu.name} onChange={e=>setNewMenu({...newMenu, name: e.target.value})} className="p-3 border rounded-lg" />
                    <input type="text" placeholder="Price (e.g., ₹450)" required value={newMenu.price} onChange={e=>setNewMenu({...newMenu, price: e.target.value})} className="p-3 border rounded-lg" />
                    <input type="text" placeholder="Category (e.g., Starters, Mains)" required value={newMenu.category} onChange={e=>setNewMenu({...newMenu, category: e.target.value})} className="p-3 border rounded-lg" />
                    <input type="text" placeholder="Description" required value={newMenu.desc} onChange={e=>setNewMenu({...newMenu, desc: e.target.value})} className="p-3 border rounded-lg" />
                    
                    <div className="sm:col-span-2 flex flex-col md:flex-row items-start md:items-center gap-4 bg-white p-3 border rounded-lg">
                      {newMenu.img ? (
                      ...
                      <div className="flex-1 w-full flex flex-col gap-2">
                        ...
                        <div className="flex flex-col xl:flex-row gap-2 w-full">
                            <input type="text" placeholder="Image URL" value={newMenu.img.startsWith('data:') ? 'Local File Uploaded' : newMenu.img} onChange={e=>setNewMenu({...newMenu, img: e.target.value})} className="flex-1 p-2 border rounded text-sm bg-gray-50" readOnly={newMenu.img.startsWith('data:')} />
                            <span className="self-center text-xs font-bold text-gray-400 hidden xl:block">OR</span>
                    ...
                    <button type="submit" className="sm:col-span-2 py-3 bg-[#5a992a] text-white rounded-lg font-bold uppercase tracking-widest hover:opacity-90 mt-2">Add Dish</button>
                 </form>
               </section>
               ...
               <section>
                 ...
                 <div className="space-y-4">
                   {data.menu.map(item => (
                     <div key={item.id} className="flex flex-col sm:flex-row gap-4 items-center bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
                       ...
                       <div className="flex items-center gap-4">
                         <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest cursor-pointer whitespace-nowrap">
                            <input type="checkbox" checked={item.showOnHome} onChange={() => toggleMenuHome(item.id)} className="w-5 h-5 accent-[#5a992a]" />
                            Show On Home
                         </label>
                         <button onClick={() => deleteMenu(item.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={20}/></button>
                       </div>
                     </div>
                   ))}
                 </div>
            ...
            <div className="space-y-10 animate-fadeIn">
               <section className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                 ...
                 <form onSubmit={handleAddGallery} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input type="text" placeholder="Category (e.g., Food, Ambiance)" required value={newGallery.category} onChange={e=>setNewGallery({...newGallery, category: e.target.value})} className="p-3 border rounded-lg sm:col-span-2" />
                    ...
                    <div className="sm:col-span-2 flex flex-col md:flex-row items-start md:items-center gap-4 bg-white p-3 border rounded-lg">
                      ...
                      <div className="flex-1 w-full flex flex-col gap-2">
                        ...
                        <div className="flex flex-col xl:flex-row gap-2 w-full">
                            <input type="text" placeholder="Image URL" value={newGallery.src.startsWith('data:') ? 'Local File Uploaded' : newGallery.src} onChange={e=>setNewGallery({...newGallery, src: e.target.value})} className="flex-1 p-2 border rounded text-sm bg-gray-50" readOnly={newGallery.src.startsWith('data:')} />
                            <span className="self-center text-xs font-bold text-gray-400 hidden xl:block">OR</span>
                    ...
                    <button type="submit" className="sm:col-span-2 py-3 bg-[#5a992a] text-white rounded-lg font-bold uppercase tracking-widest hover:opacity-90 mt-2">Add to Gallery</button>
                 </form>
               </section>
               ...
               <section>
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                   {data.gallery.map(img => (
                     <div key={img.id} className="relative group rounded-xl overflow-hidden border border-gray-200 h-40">
